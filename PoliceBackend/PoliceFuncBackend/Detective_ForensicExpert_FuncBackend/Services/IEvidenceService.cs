...
namespace PoliceFuncBackend.Services
{
    public interface IEvidenceService
    {
        ...
        Task<Evidence?> AssignAnalystAsync(int id, AssignAnalystDto dto);
        Task<Evidence?> UpdateStatusAsync(int id, UpdateEvidenceStatusDto dto);
        Task<string?> GetChainOfCustodyAsync(int id);
        Task<Evidence?> AddChainOfCustodyAsync(int id, ChainOfCustodyDto dto);
    }
}